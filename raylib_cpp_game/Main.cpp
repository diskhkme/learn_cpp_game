...
void UpdatePlayer(float& playerPositionX, float& playerPositionY)
{
...
void UpdateEnemy(float enemySpeed, float* enemyPositionX, float* enemyPositionY, int enemyCount, float playerPositionX, float playerPositionY)
{
	...
	{
		float vectorX = playerPositionX - enemyPositionX[i]; // enemy에서 player로 향하는 벡터의 x방향 성분
		float vectorY = playerPositionY - enemyPositionY[i]; // enemy에서 player로 향하는 벡터의 y방향 성분
		...
		enemyPositionX[i] += vectorX * enemySpeed; // speed 만큼의 속도로 player쪽으로 이동하게 됩니다
		enemyPositionY[i] += vectorY * enemySpeed; 
	}
...
void DrawEnemy(float* enemyPositionX, float* enemyPositionY, int enemyCount, float enemyRadius, Color enemyColor)
{
	for (int i = 0; i < enemyCount; i++)
	{
...
int main() {
	...
	const float playerWidth = 20; // 플레이어 사각형의 크기는 변하지 않는다고 합시다.
	...
	while (!WindowShouldClose())
	{
		...
		{ // 데이터 조작 (Update)
			...
			UpdatePlayer(playerPositionX, playerPositionY);

			// 이제 적들도 움직입니다.
			UpdateEnemy(enemySpeed, enemyPositionX, enemyPositionY, enemyCount, playerPositionX, playerPositionY);
		}
...
// 1. 적들의 이동속도를 더 빠르게, 혹은 더 느리게 바꿔 보세요
// 2. 시간이 지나면 적들이 겹쳐 버립니다. 어떻게 해결할 수 있을까요? 
