#include "raylib.h" // 화면을 그리는 데 필요한 기능들이 구현되어 있는 헤더 파일
...
int main() {
	...
	// 윈도우 만들기
	InitWindow(screenWidth, screenHeight, "0_Introduction"); // Parameter: ((int)가로, (int)세로, (std::string)창 이름)
	...
	while (!WindowShouldClose())
	{
		...
		ClearBackground(backgroundColor); // 배경을 위에 설정한 backgroundColor값으로 칠합니다.
